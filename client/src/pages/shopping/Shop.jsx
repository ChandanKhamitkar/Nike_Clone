...
function Shop() {
  ...
  const handleFavourite = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${baseURL}/api/user/addToFavourite`,
        addBagItem
      );
      ...
      <div className="flex justify-center items-center mx-auto my-16 space-x-16 md:flex-col md:space-x-0 md:mx-6">
        <div className="hidden space-y-7 md:block md:self-start">
          ...
          <div>
            ...
            <p className="text-gray-400 font-medium">
              Also inlcude all aplicalbe duties
            </p>
          </div>
        </div>
        ...
        <div className="max-w-[400px] space-y-7 self-start md:max-w-full mobile:mx-auto">
          ...
          <div className="space-y-2">
            <p className="flex justify-between px-2 font-medium">
            ...
            <div className="flex flex-wrap gap-2">
              {productInfo.availableSize.map((size, index) => (
                <p
                  key={index}
                  htmlFor=""
                  className={`px-8 py-4 rounded-md border w-fit cursor-pointer hover:border-gray-950 ${
                    selected === index ? "border-gray-950" : "border-gray-300"
                  }`}
          ...
          <div className="flex flex-col space-y-3">
            <button
              onClick={handleBag}
              className="px-9 py-3 rounded-3xl font-semibold bg-black text-white  sm:text-sm hover:bg-gray-800 "
            >
              Add to Bag
            </button>
            <button
              onClick={handleFavourite}
              className="px-9 py-2 rounded-3xl font-semibold bg-transparent text-black border-2  sm:text-sm flex justify-center items-center space-x-2 hover:border-black"
            >
              <span>Favourite</span>{" "}
              <svg
          ...
          <div>
            ...
            <div className="my-7">
              <p className="flex justify-between px-3 my-4 items-center">
                <span className="text-xl">Size & Fit</span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  ...
                  height="18"
                  fill="black"
                  className="bi bi-caret-down cursor-pointer"
                  viewBox="0 0 16 16"
                >
              ...
              <ul
                className={`ml-4 list-disc text-sm space-y-2 ${
                  showSize ? "animate-fadeIn" : "hidden"
                }`}
                style={{
                  transition: "height 0.5s",
                  height: showSize ? "auto" : "0",
                }}
              >
                <li>
                  Fits large : we recommend you to order half size a down.
                </li>
                <li>Size Guide</li>
              </ul>
            </div>
            ...
            <div className="my-7">
              <p className="flex justify-between px-3 my-4 items-center">
                <span className="text-xl">Delivery & Returns</span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <ul
                className={`ml-4 list-disc text-sm space-y-2 ${
                  delivery ? "animate-fadeIn" : "hidden"
                }`}
                style={{
                  transition: "height 0.5s",
                  height: delivery ? "auto" : "0",
                }}
              >
                <li>All purchases are subject to delivery fees.</li>
                <li>Standard delivery 4 to 9 business days</li>
                <li>
                  Orders are processed and delivered Monday to Friday excluding
                  public holidays
                </li>
              </ul>
            </div>
